...
class Recruitomatic9003(commands.Cog):
    ...
    async def run_cycle(self, ctx, user_settings, view):
        ...
        if not nations:
            ...
            embed.description = "Please click each batch, then the approve button, to get credit and receive the next set\nWhen you are all done recruiting for the day, click \'all done\'.  \nFor your convenience, I have a timer funciton you can use.  Just set it up by doing [p]set_timer {Num of Seconds}."
    ...
    async def Thanks_9003(self, ctx):
        await ctx.send("Your appreciation is appreciated! if this has been a useful tool, please let 9003/9006 know by sending them a TG or a discord message. The wellspring starts on the exluded region list, another way you can say thanks is by leaving it on there!")
